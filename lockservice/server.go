...
type LockServer struct {
	...
	backupActive bool   // is backup active?

	// for each lock name, is it locked?
	locks map[string]bool
	// cache reply boolean for each request
...
func (ls *LockServer) Lock(args *LockArgs, reply *LockReply) error {
	...
	if ls.am_primary == true && ls.backupActive == true {
		ls.backupActive = call(ls.backup, "LockServer.Lock", args, reply)
	}
...
func StartServer(primary string, backup string, am_primary bool) *LockServer {
	...
	ls.UUIDs = map[string]bool{}
	ls.backupActive = true
